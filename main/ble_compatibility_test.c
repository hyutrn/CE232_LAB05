...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param) {
    printf("Start Write\n");
    uint8_t write_data[SHORT_CHAR_VAL_LEN];
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC && prepare_write_env->prepare_buf) {
        uint8_t data_len = prepare_write_env->prepare_len;
        memcpy(write_data, prepare_write_env->prepare_buf, data_len);

        // Print the data received
        ESP_LOGI(EXAMPLE_TAG, "(3) ***** long write data received ***** \n");
        ESP_LOG_BUFFER_HEX(EXAMPLE_TAG, write_data, data_len);
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            memcpy(word_transfer, param->write.value, length_word_transfer);
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle/*  && param->write.len == 4 */) {
                    uint8_t write_data[SHORT_CHAR_VAL_LEN] = {};
                    uint8_t data_len = param->write.len;
                    memcpy(write_data, param->write.value, data_len);
...
void connect_BLE(void) {
    ...
    // Log ra màn hình
    ESP_LOGI(EXAMPLE_TAG, "BLE is connected");
}

void disconnect_BLE(void) {
    // esp_bluedroid_disable();      // Tắt Bluetooth stack
    // esp_bluedroid_deinit();       // Giải phóng tài nguyên Bluetooth stack
    esp_bt_controller_disable();  // Tắt Bluetooth controller
    // esp_bt_controller_deinit();
    // Log ra màn hình
    ESP_LOGI(EXAMPLE_TAG, "BLE is disconnected");
}
*/
void init_BLE(void)
{
